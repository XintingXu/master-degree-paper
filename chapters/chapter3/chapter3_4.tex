...
如表\nref{tab:3:entropy-range}，K-L散度主要用于检测IPD分布及突发丢包长度分布。在\nref{chap:analyze:results:ipd}及\nref{chap:analyze:results:burst}分析中，IPD分布及突发丢包长度的统计频率与概率分布相近，K-L散度能够进行一致性判别；而区间丢包数的概率质量函数不稳定，通过K-L散度对概率质量函数进行判断准确率较低。
