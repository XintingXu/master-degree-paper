	...
	\begin{table}[]
      ...
      \label{tab:3:distance-range}
          \begin{tabular*}{0.8\textwidth}{@{\extracolsep{\fill}}ccc}
            ...
            检验方法 & 测试对象 & 通过条件 \\ 
            \midrule
            \multirow{3}{*}{Wasserstein距离} & IPD分布 & \multirow{3}{*}{$D_{w}(F(x),G(x))\times n<1$} \\ 
            & 突发丢包长度分布 \\
            & 区间丢包分布 \\
            \\
            \multirow{3}{*}{能量距离} & IPD分布 & \multirow{3}{*}{$D_{e}(F(x),G(x))<1$} \\ 
            & 突发丢包长度分布 \\
            & 区间丢包分布 \\
            \bottomrule
          \end{tabular*}
...
基于相对距离的检测方法，适用的测试对象及通过条件，如表\nref{tab:3:distance-range}。通常$D_{w}(F(x),G(x))\times n$及$D_{e}(F(x),G(x))$与一致性概率无关，在本检测方法中，设定上限为1，如果距离计算结果超过1，则认定分布不一致。
