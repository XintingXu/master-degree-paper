...
类似K-S检验，Welch's t检验得到的$p$值，反映了样本的一致性水平，一般取0.05作为分界值，对应两个样本分布一致的概率为95\%。由于Welch's t检验适用范围的限制，参照\nref{chap:analyze:results}中的分析结果，判定Welch's t检验适用于IPD分布。
	...
	\begin{table}
        ...
        \begin{threeparttable}
            \begin{tabular*}{0.85\textwidth}{@{\extracolsep{\fill}}ccc}
              ...
              Welch's t检验 & IPD分布 & p值满足$p\ >\ 0.05$ \\ 
              Mann–Whitney rank检验 & IPD分布 & p值满足$p\ >\ 0.05$ \\ 
              \bottomrule
            \end{tabular*}
            \begin{tablenotes}
              \footnotesize
...
\insertTable{
    ...
    \begin{threeparttable}
      ...
      \begin{tablenotes}
        \footnotesize
        \item[] 量化测试项为$2 \sim 6,\ 8 \sim 10,\ 12 \sim 13$
      \end{tablenotes}
    \end{threeparttable}
  \end{table}
}
