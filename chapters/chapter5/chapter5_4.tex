...
\insertContents{
    ...
    \label{alg:5:codeword-generation}
        \begin{algorithmic}[1]
            \REQUIRE $message$, $BL$, $L_{HASH}$, $L_{CRC}$, $R$, $Salt$, $Seed$
            \ENSURE $C \leftarrow \{\}$
            \STATE $D \leftarrow$ group into blocks$(message,\ BL)$;
            \STATE $modulated\_sections \leftarrow NULL$;
            \STATE $salt \leftarrow Salt\ \oplus\ Seed$
            \FOR{$D_i$\ in\ $D$}
                \STATE $j \leftarrow \left \lfloor{\frac{i}{R - 1}} \right \rfloor \times R + (i - 1) \% R + 1$;
                \STATE append\ $D_i$\ to\ $modulated\_sections$;
                \STATE $h_j \leftarrow L_{HASH}$\ bits\ of\ HASH($salt//modulated\_sections//salt$);
                \STATE $C_j \leftarrow$ append\ $h_j$\ to\ $D_i$;
                \STATE append\ $h_j$\ to\ $modulated\_sections$;
                \STATE append\ $C_j$\ to\ $C$;
                \IF{$i$\ mod\ $R$\ ==\ 0}
                    \STATE $C_{j + 1} \leftarrow (L_{Codeword} - L_{CRC})$\ bits of HASH($salt//modulated\_sections//salt$);
                    \STATE append\ $C_{j + 1}$\ to\ $C$;
                    \STATE $modulated\_sections \leftarrow NULL$;
                \ENDIF
            \ENDFOR
            \FOR{$C_j$ in $C$}
                \STATE $C_j \leftarrow$\ append\ $L_{CRC}$\ bits of CRC32($salt//C_jsalt//$)\ to\ $C_j$;
            \ENDFOR
            \RETURN $C$
        \end{algorithmic}
    \end{algorithm}
...
\insertContents{
    ...
    \label{alg:5:codeword-identification}
        \begin{algorithmic}[1]
            \REQUIRE $S'$, $L_{CRC}$, $Salt$, $Seed$
            \ENSURE $C' \leftarrow \{\}$
            \STATE $salt \leftarrow Salt\ \oplus\ Seed$
            \FOR{\{${S_j'}$\} in $S'$}
                \STATE $\{C_j'\} \leftarrow \{\}$
                \FOR{$S_j'$\ in\ \{$S_j'$\}}
                    \STATE $v_j', h_j', D_j' \leftarrow$ extracted from $S_j'$
                    \STATE $crc32\_result \leftarrow L_{CRC}$ bits of CRC32($salt//D_j'//h_j'//salt$)
                    \IF{$v_i'$ == $crc32\_result$}
                        \STATE append\ $S_j'$\ to\ \{$C_j'$\}
                    \ENDIF
                \ENDFOR
                \STATE append\ \{$C_j'$\}\ to\ $C'$
            \ENDFOR
...
\insertContents{
    ...
    \label{alg:5:codeword-to-sequence}
        \begin{algorithmic}[1]
            \REQUIRE $C$, $L_{Codeword}$, $M_{cols}$, $Salt$, $Seed$
            \ENSURE $P \leftarrow \{\}$
            \STATE $S \leftarrow \{\}$
            \STATE $offset \leftarrow$ Random($Salt \oplus Seed$)
            \FOR{$C_j$ in $C$}
                \STATE $S_k \leftarrow$ Integer($C_j$) + 1
                \STATE append\ $S_k$\ to\ $S$
                \IF{$j$\ mod\ 2\ ==\ 0}
                    \STATE append\ $S_k\oplus S_{k-1}$\ to $S$
                \ENDIF
            \ENDFOR
            \FOR{$S_k$\ in\ $S$}
                \STATE $offset \leftarrow$ Random($offset$)
                \STATE $S_k \leftarrow (S_k + offset) mod (2^{L_{Codeword}}) + 1$
            \ENDFOR
            \FOR{$S_k$\ in\ $S$}
                \STATE $P_k \leftarrow P_k(k, S_k)$
                \STATE append\ $P_k$\ to\ $P$
            \ENDFOR
            \RETURN $P$
        \end{algorithmic}
    \end{algorithm}
